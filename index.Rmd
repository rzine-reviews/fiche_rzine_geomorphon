...
- un bassin versant de moyenne montagne : L'Allan de la Savoureuse à la Lizaine
- un bassn versant de plaine : L'Yvette de sa source au confluent de la Mérantaise (inclus)

# Méthodologie

## Création du raster virtuel

To do : télécharger les données RGE 5m pour l'Yvette et pour le BV de l'Allan.


```
ras_virt <- list.files(c, full.names = T, pattern = ".asc$")    # sélection de toutes les dalles
terra::vrt(ras_virt, v, overwrite = T)                          # création du raster virtuel
ras <- raster(v)
plot(ras)
```
...
```
mnt <- raster("bv_riberot.tif")
dem_geomorphons = qgis_run_algorithm("saga:geomorphons", # attention, il faut que l'extension soit active dans QGIS
                                     DEM = mnt,
                                     THRESHOLD = 1,
                                     RADIUS = 5,
                                     METHOD = 1,
                                     GEOMORPHONS = paste("geomorphon_riberot_5m_LD5", ".tif"))
                                     
geomorphon <- raster("geomorphon_riberot_5m_LD5.tif")
report_geomorphons = qgis_run_algorithm("native:rasterlayeruniquevaluesreport",
                                    INPUT = geomorphon,
                                    BAND = 1,
                                    OUTPUT_TABLE = paste("rapport_geomorphon_riberot_5m_LD5", ".csv"))
                                    
stat <- read.csv("rapport_geomorphon_riberot_5m_LD5.csv")
data.frame(stat)
```
